...
usage = ['двач - случайная фотка с двача',
       'мемы - присылает мем из 4ch',
	   'мем - прислылает мем из овсянки',
	   'мемас - присылает мем из щебестана']
...
@plugin.on_command('мемы', 'мемасики', 'мемчики', 'мемасик', 'мемосы', 'мемасы', '4ch')
async def just_memes(msg, args):
    group_id = -45745333
    await give_memes(msg, group_id)
	
@plugin.on_command('мем')
async def ovs_memes(msg, args):
    group_id = -66678575
    await give_memes(msg, group_id)
	
@plugin.on_command('мемас')
async def scheb_memes(msg, args):
    group_id = -73598440
    await give_memes(msg, group_id)
