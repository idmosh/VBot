...
# Префиксы сообщений, с помощью которых бот будет понимать, что обращаются к нему. ВНИМАНИЕ! Регистр тоже учитывается
PREFIXES = ('бот ', 'бот,', 'бот, ', 'Бот ', 'Бот,', 'Бот, ')
...
CAPTCHA_SERVER = "server"  # Сервис для решения капч.
CAPTCHA_KEY = "key"  # API ключ для сервиса решения капч
